...
import {
  createHash, generateKeyPairSync, sign, verify as edVerify,
...
export function verifyReceipt(pr: PortableReceipt): boolean {
  try {
    const der = Buffer.concat([Buffer.from("302a300506032b6570032100", "hex"), Buffer.from(pr.publicKey, "base64")]);
    const key = createPublicKey({ key: der, format: "der", type: "spki" });
    return edVerify(null, Buffer.from(canonical(pr.receipt)), key, Buffer.from(pr.signature, "base64"));
  } catch {
    return false;
  }
}
